...
function getOpenCodeDir(global: boolean): string {
  if (global) {
...
function getAgentsMdPath(global: boolean): string {
  return join(getOpenCodeDir(global), 'AGENTS.md');
}
...
export const opencodeHarness: Harness = {
  ...
  async detect(): Promise<boolean> {
    ...
    const localConfig = resolve(process.cwd(), 'opencode.json');
  ...
  getConfigDir(global: boolean): string {
    return getOpenCodeDir(global);
  },

  async install(global: boolean): Promise<InstallResult[]> {
    const results: InstallResult[] = [];
    const configDir = getOpenCodeDir(global);
